...
func AddNewsDetail(w http.ResponseWriter, r *http.Request) {
	...
	contentEN, content_checksumEN, err := UploadFile(r, "contentEN", "NewsInfo", EN)
	if err != nil {
		panic(err)
	}
	contentIN, content_checksumIN, err := UploadFile(r, "contentIN", "NewsInfo", IN)
	if err != nil {
		panic(err)
	}
	...
	_, err = stmt3.Exec(name, release_date, news_type)
	if err != nil {
		panic(err)
	}
