...
func GetGuildMissionContributionLog(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT a.guild_mission_contribution_log, a.user_id, u.user_name, a.guild_mission_detail_id, e.description, a.room_id, a.guild_party_id, a.amount, a.contribution_date FROM lokapala_guilddb.t_guild_mission_contribution_log a LEFT JOIN lokapala_accountdb.t_user u ON a.user_id = u.user_id LEFT JOIN lokapala_guilddb.t_guild_mission_detail b ON a.guild_mission_detail_id = b.guild_mission_detail_id LEFT JOIN lokapala_guilddb.t_mission_detail c ON b.mission_detail_id = c.mission_detail_id LEFT JOIN lokapala_guilddb.t_mission e ON c.mission_id = e.mission_id where a.guild_mission_detail_id = ?", guild_mission_id)
	if err != nil {
		panic(err)
	}
