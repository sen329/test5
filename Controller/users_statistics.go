...
func GetKsaStatCount(w http.ResponseWriter, r *http.Request) {
	...
	for result.Next() {
		...
		err := result.Scan(&stat.Ksatriya_id, &stat.Ksatriya_name, &stat.Match_count, &stat.Win_count, &stat.Win_rate, &stat.Lose_count, &stat.Lose_rate)
		if err != nil {
			panic(err.Error())
		}
...
func GetMatchData(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT trr.game_duration as game_duration, trr.game_mode as game_mode, COALESCE(trr.match_id, 0) as match_id, CONVERT_TZ(tpr.start_time, 'SYSTEM', '+07:00') as start_time, trrs.*, tu.user_name as user_name, tu.avatar_icon as avatar_icon, tu.frame as frame FROM (SELECT t.*, tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot t LEFT JOIN lokapala_accountdb.t_ksatriya tk ON t.ksatriya_id = tk.ksatriya_id where t.room_id = ?) trrs JOIN (SELECT * FROM lokapala_roomdb.t_room_result t where t.room_id = ?) trr ON trrs.room_id = trr.room_id JOIN (SELECT room_id, start_time FROM lokapala_roomdb.t_past_room t where t.room_id = ?) tpr ON trrs.room_id = tpr.room_id JOIN (SELECT user_id, user_name, avatar_icon, frame FROM lokapala_accountdb.t_user) tu ON trrs.user_id = tu.user_id;", room_id, room_id, room_id)
	if err != nil {
		panic(err.Error())
	}
...
func GetUserMatchHistory(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT t.room_id as room_id, game_duration, t.game_mode as game_mode, CONVERT_TZ(tpr.start_time, 'SYSTEM', '+07:00') as start_time, win, ksatriya_id, `level`, `kill`, death, assist, if(mvp_badge>3, 1, 0) as mvp_badge, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=0) as s0_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=0) as s0_ksa_name, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=1) as s1_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=1) as s1_ksa_name, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=2) as s2_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=2) as s2_ksa_name, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=3) as s3_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=3) as s3_ksa_name, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=4) as s4_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=4) as s4_ksa_name, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=5) as s5_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=5) as s5_ksa_name, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=6) as s6_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=6) as s6_ksa_name, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=7) as s7_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=7) as s7_ksa_name, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=8) as s8_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=8) as s8_ksa_name, (select ksatriya_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=9) as s9_ksa, (select tk.ksatriya_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_ksatriya tk ON trrs.ksatriya_id = tk.ksatriya_id where trrs.room_id=t.room_id and slot_index=9) as s9_ksa_name, (SELECT SUM(case when slot_index<5 then `kill` end) FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id) as blue_kill, (SELECT SUM(case when slot_index>=5 then `kill` end) FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id) as red_kill, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=0) as s0_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=0) as s0_user_name, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=1) as s1_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=1) as s1_user_name, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=2) as s2_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=2) as s2_user_name, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=3) as s3_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=3) as s3_user_name, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=4) as s4_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=4) as s4_user_name, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=5) as s5_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=5) as s5_user_name, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=6) as s6_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=6) as s6_user_name, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=7) as s7_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=7) as s7_user_name, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=8) as s8_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=8) as s8_user_name, (select trrs.user_id FROM lokapala_roomdb.t_room_result_slot trrs where trrs.room_id=t.room_id and slot_index=9) as s9_user, (select tu.user_name FROM lokapala_roomdb.t_room_result_slot trrs LEFT JOIN lokapala_accountdb.t_user tu ON trrs.user_id = tu.user_id where trrs.room_id=t.room_id and slot_index=9) as s9_user_name FROM (SELECT trrs.*, trr.game_mode, trr.game_duration FROM (SELECT * FROM lokapala_roomdb.t_room_result_slot ttrrs where if(? = 0, TRUE, ttrrs.user_id = ?)) trrs JOIN (SELECT ttrr.* FROM lokapala_roomdb.t_room_result ttrr where if(? = 0, TRUE, ttrr.game_mode = ?)) trr ON trrs.room_id = trr.room_id where NOT COALESCE(trr.match_id, 0) = 0 ORDER BY trrs.room_id DESC LIMIT ? OFFSET ?) t LEFT JOIN (SELECT room_id, start_time FROM lokapala_roomdb.t_past_room) tpr ON t.room_id = tpr.room_id;", user_id, user_id, game_mode, game_mode, count, offset)
	if err != nil {
		panic(err.Error())
	}
	...
	for result.Next() {
		...
		err := result.Scan(&stat.Room_id, &stat.Game_duration, &stat.Game_mode, &stat.Start_time, &stat.Win, &stat.Ksatriya_id, &stat.Level, &stat.Kill, &stat.Death, &stat.Assist, &stat.Mvp_badge, &stat.Slot0_ksa, &stat.Slot0_ksa_name, &stat.Slot1_ksa, &stat.Slot1_ksa_name, &stat.Slot2_ksa, &stat.Slot2_ksa_name, &stat.Slot3_ksa, &stat.Slot3_ksa_name, &stat.Slot4_ksa, &stat.Slot4_ksa_name, &stat.Slot5_ksa, &stat.Slot5_ksa_name, &stat.Slot6_ksa, &stat.Slot6_ksa_name, &stat.Slot7_ksa, &stat.Slot7_ksa_name, &stat.Slot8_ksa, &stat.Slot8_ksa_name, &stat.Slot9_ksa, &stat.Slot9_ksa_name, &stat.Blue_kill, &stat.Red_kill, &stat.Slot0_user, &stat.Slot0_user_name, &stat.Slot1_user, &stat.Slot1_user_name, &stat.Slot2_user, &stat.Slot2_user_name, &stat.Slot3_user, &stat.Slot3_user_name, &stat.Slot4_user, &stat.Slot4_user_name, &stat.Slot5_user, &stat.Slot5_user_name, &stat.Slot6_user, &stat.Slot6_user_name, &stat.Slot7_user, &stat.Slot7_user_name, &stat.Slot8_user, &stat.Slot8_user_name, &stat.Slot9_user, &stat.Slot9_user_name)
		if err != nil {
			panic(err.Error())
		}
